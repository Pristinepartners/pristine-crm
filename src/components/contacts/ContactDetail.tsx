...
export function ContactDetail({
  ...
  const getOutcomeColor = (outcome: string) => {
    const colors: Record<string, string> = {
      ...
      'No Answer': 'bg-gray-100 text-[var(--color-text)]',
    ...
    return colors[outcome] || 'bg-gray-100 text-[var(--color-text)]'
  }
  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between">
        <Link
          href="/contacts"
          className="inline-flex items-center gap-2 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition"
        >
        ...
        {!isEditing && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-stone-100 rounded-lg transition"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            {isEditing ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold text-[var(--color-text)]">Edit Contact</h2>
                  <button
                    onClick={handleCancelEdit}
                    className="p-1 text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] rounded transition"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--color-text)] mb-1">Business Name</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text)] mb-1">Postal Code</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--color-text)] mb-1">LinkedIn URL</label>
                  <input
                    type="url"
              ...
              <>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h1 className="text-xl font-bold text-[var(--color-text)]">{contact.name}</h1>
                ...
                <div className="flex gap-2 mb-6">
                  {contact.phone && (
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2 text-sm bg-stone-100 text-[var(--color-text)] rounded-lg hover:bg-stone-200 transition"
                ...
                <div className="space-y-3">
                  ...
                  {contact.email && (
                    <div className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                      ...
                      <a href={`mailto:${contact.email}`} className="hover:text-[var(--color-primary)]" style={{ transition: 'color 0.15s' }}>
                        {contact.email}
                      </a>
                    </div>
                  )}
                  {contact.phone && (
                    <div className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                      ...
                      <a href={`tel:${contact.phone}`} className="hover:text-[var(--color-primary)]" style={{ transition: 'color 0.15s' }}>
                        {contact.phone}
                      </a>
                    </div>
                  )}
                  ...
                  {contact.website && (
                    <div className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                      ...
                      <a
                        ...
                        className="hover:text-[var(--color-primary)] flex items-center gap-1" style={{ transition: 'color 0.15s' }}
                      >
                    ...
                    <div className="flex items-center gap-3 text-[var(--color-text-secondary)]">
                      ...
                      <span className="text-sm">
                        ...
                        {daysSinceLastContact !== null && (
                          <span className="text-[var(--color-text-muted)] ml-1">({daysSinceLastContact} days ago)</span>
                        )}
                      </span>
                    </div>
                  )}
                  <div className="flex items-center gap-3 text-[var(--color-text-secondary)] text-sm pt-2">
                ...
                <div className="mt-6 pt-6 border-t border-[var(--color-border)]">
                  <h3 className="text-sm font-medium text-[var(--color-text)] mb-2">Lead Score</h3>
                  <LeadScoreBreakdown
                ...
                {contact.notes && (
                  <div className="mt-6 pt-6 border-t border-[var(--color-border)]">
                    ...
                    <p className="text-[var(--color-text-secondary)] text-sm whitespace-pre-wrap">{contact.notes}</p>
                  </div>
                )}
              </>
            )}
          ...
          {/* Opportunities */}
          <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
            <h2 className="text-lg font-semibold text-[var(--color-text)] mb-4">Pipeline</h2>
            {currentOpportunities.length === 0 ? (
              <p className="text-[var(--color-text-secondary)] text-sm">Not in any pipeline</p>
            ) : (
              <div className="space-y-3">
                  ...
                  <div key={opp.id}>
                    {editingOppId === opp.id ? (
                      <div className="p-3 bg-stone-50 rounded-lg space-y-3">
                        <div>
                          <label className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1">Pipeline</label>
                          <select
                            value={oppPipelineId}
                            onChange={(e) => {
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="flex-1 px-3 py-1.5 border border-[var(--color-border-strong)] text-[var(--color-text)] rounded-lg hover:bg-stone-100 transition text-sm"
                          >
                            Cancel
                          </button>
                          <button
                      ...
                      <div className="p-3 bg-stone-50 rounded-lg">
                        <div className="flex items-start justify-between">
                          <Link href={`/pipelines/${opp.pipeline_id}`} className="flex-1">
                            <div className="font-medium text-[var(--color-text)] hover:text-[var(--color-primary)]">{opp.pipeline?.name}</div>
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-primary)] hover:bg-amber-50/50 rounded transition"
                              title="Edit"
                            >
                              <Pencil className="w-4 h-4" />
                            </button>
                            <button
                              ...
                              className="p-1.5 text-[var(--color-text-muted)] hover:text-red-600 hover:bg-red-50 rounded transition"
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[var(--color-text)]">Appointments</h2>
              <button
            ...
            {appointments.length === 0 ? (
              <p className="text-[var(--color-text-secondary)] text-sm">No appointments scheduled</p>
            ) : (
              <div className="space-y-3">
                {appointments.map((apt) => (
                  <div key={apt.id} className="p-3 bg-stone-50 rounded-lg">
                    <div className="font-medium text-[var(--color-text)]">{apt.title}</div>
                    <div className="text-sm text-[var(--color-text-secondary)] mt-1">
                      {format(new Date(apt.datetime), 'MMM d, yyyy h:mm a')}
                    </div>
                    {apt.location && (
                      <div className="text-sm text-[var(--color-text-secondary)]">{apt.location}</div>
                    )}
                    <span
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold text-[var(--color-text)]">Activity Timeline</h2>
              <button
