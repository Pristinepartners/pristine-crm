...
export function DemoSidebar({ account, userEmail }: DemoSidebarProps) {
  const pathname = usePathname()
  const router = useRouter()
  const [showDropdown, setShowDropdown] = useState(false)
  const dropdownRef = useRef<HTMLDivElement>(null)

  const basePath = `/demo/${account.id}`
  ...
  const handleAccountSwitch = (accountId: string) => {
    setShowDropdown(false)
    // Navigate to the same sub-page on the new account
    const currentSubPath = pathname.replace(`/demo/${account.id}`, '')
    router.push(`/demo/${accountId}${currentSubPath}`)
  }

  return (
    <aside className="w-64 flex flex-col" style={{ backgroundColor: account.secondaryColor }}>
      {/* Back to Admin */}
          ...
          >
            ...
            >
              <span
                ...
                style={{ color: account.primaryColor, fontFamily: 'var(--font-playfair), Playfair Display, Georgia, serif' }}
              >
                {account.logoInitial}
              </span>
            </div>
              ...
              {DEMO_ACCOUNTS.map((acc) => (
                ...
                >
                  <div
                    ...
                    style={{ border: `1px solid ${acc.primaryColor}` }}
                  >
                    <span className="text-xs font-medium" style={{ color: acc.primaryColor }}>
                      {acc.logoInitial}
                    </span>
                  </div>
                  <div className="flex-1 text-left min-w-0">
                  ...
                  {acc.id === account.id && (
                    <Check className="w-4 h-4 flex-shrink-0" style={{ color: acc.primaryColor }} />
                  )}
                </button>
              ))}
            </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              style={isActive ? {
                backgroundColor: `${account.primaryColor}15`,
      ...
      <div className="p-4" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="px-3">
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: account.primaryColor }} />
            Demo Mode
          </div>
