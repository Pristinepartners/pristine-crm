...
function KPICard({
  ...
  const colorClasses = {
    blue: 'bg-amber-50 text-[var(--color-primary)]',
    ...
    <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          {subValue && <p className="text-sm text-[var(--color-text-secondary)] mt-1">{subValue}</p>}
          {trend && trendValue && (
            <div className={`flex items-center gap-1 mt-2 text-sm ${
              trend === 'up' ? 'text-green-600' : trend === 'down' ? 'text-red-600' : 'text-[var(--color-text-secondary)]'
            }`}>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
      ...
      <div className="space-y-3">
        ...
        {entries.length === 0 && (
          <p className="text-[var(--color-text-secondary)] text-sm text-center py-4">No data available</p>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-[var(--color-text)]">{data.pipeline}</h3>
        <div className="text-right">
          <p className="text-sm text-[var(--color-text-secondary)]">{data.total} opportunities</p>
      ...
      <div className="space-y-2">
        {data.stages.map((stage, index) => (
          <div key={stage.stage} className="relative">
            <div
              className="h-10 bg-[var(--color-primary)] rounded-lg flex items-center justify-between px-3 text-white text-sm"
              style={{
...
export function ReportsDashboard({ data }: ReportsDashboardProps) {
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[var(--color-text)]">Reports & KPIs</h1>
        <p className="text-[var(--color-text-secondary)] mt-1">Track your performance metrics</p>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6 mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-stone-50 rounded-lg">
                <p className="text-2xl font-bold text-[var(--color-text)]">{data.totalContacts}</p>
                <p className="text-sm text-[var(--color-text-secondary)]">Total Leads</p>
              </div>
              <div className="text-center p-4 bg-stone-50 rounded-lg">
                <p className="text-2xl font-bold text-[var(--color-text)]">{data.totalActivities}</p>
                <p className="text-sm text-[var(--color-text-secondary)]">Total Calls</p>
              </div>
              <div className="text-center p-4 bg-stone-50 rounded-lg">
                <p className="text-2xl font-bold text-[var(--color-text)]">{data.appointmentsBooked}</p>
                <p className="text-sm text-[var(--color-text-secondary)]">Appointments Set</p>
              </div>
              <div className="text-center p-4 bg-stone-50 rounded-lg">
                <p className="text-2xl font-bold text-[var(--color-text)]">{data.appointmentsShowedUp}</p>
                <p className="text-sm text-[var(--color-text-secondary)]">Showed Up</p>
              </div>
            </div>
          </div>
        ...
        <>
          ...
          {data.opportunitiesByPipeline.length > 0 ? (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-12 text-center">
              <Briefcase className="w-12 h-12 mx-auto text-[var(--color-text-muted)] mb-4" />
              <p className="text-[var(--color-text-secondary)]">No pipeline data available</p>
            </div>
          )}
        </>
      )}
      ...
      {activeTab === 'activities' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <BarChart data={data.activityByOutcome} title="Activities by Outcome" />
            <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
              <h3 className="font-semibold text-[var(--color-text)] mb-4">Conversion Funnel</h3>
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <div className="w-full bg-[var(--color-primary)] text-white rounded-lg py-3 px-4 flex justify-between">
                    <span>Total Activities</span>
                    <span className="font-bold">{data.totalActivities}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-3/4 bg-[var(--color-primary)] text-white rounded-lg py-3 px-4 flex justify-between opacity-80">
                    <span>Meetings Booked</span>
                    <span className="font-bold">{data.activityByOutcome['Meeting Booked'] || 0}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-1/2 bg-[var(--color-primary)] text-white rounded-lg py-3 px-4 flex justify-between opacity-60">
                    <span>Showed Up</span>
      ...
      {activeTab === 'team' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
              <h3 className="font-semibold text-[var(--color-text)] mb-4">Team Leaderboard</h3>
              <div className="space-y-4">
                  ...
                  .map(([owner, count], index) => (
                    <div key={owner} className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        <p className="font-medium text-[var(--color-text)] capitalize">{owner}</p>
                        <p className="text-sm text-[var(--color-text-secondary)]">{count} activities</p>
                      </div>
                      {index === 0 && (
                ...
                {Object.keys(data.activityByOwner).length === 0 && (
                  <p className="text-[var(--color-text-secondary)] text-sm text-center py-4">No team data available</p>
                )}
              </div>
            </div>
          </div>
          ...
          {/* Team Stats Grid */}
          <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)] p-6">
            <h3 className="font-semibold text-[var(--color-text)] mb-4">Team Statistics</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--color-border)]">
                    <th className="text-left py-3 px-4 text-sm font-medium text-[var(--color-text-secondary)]">Team Member</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-[var(--color-text-secondary)]">Activities</th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-[var(--color-text-secondary)]">% of Total</th>
                  </tr>
                </thead>
                <tbody>
                  {Object.entries(data.activityByOwner).map(([owner, count]) => (
                    <tr key={owner} className="border-b border-[var(--color-border)]">
                      <td className="py-3 px-4 font-medium text-[var(--color-text)] capitalize">{owner}</td>
                      <td className="py-3 px-4 text-right text-[var(--color-text-secondary)]">{count}</td>
